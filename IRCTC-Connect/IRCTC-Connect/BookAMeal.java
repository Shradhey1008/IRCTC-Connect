...
public class BookAMeal extends JFrame implements ActionListener {
    ...
    public void showPnrDetails(PnrEnquiryFinalInfo passengerDetails) {
        ...
        bookMeal = new JButton[noOfPassengers];
        ...
        for (int i = 0; i < noOfPassengers; i++) {
            ...
            x = 200;
            Integer count = i + 1;
            bookMeal[i] = new JButton("Book Meal for Passenger " + count.toString());
            bookMeal[i].setFont(new Font("Times new roman", Font.BOLD, 15));
            bookMeal[i].setBounds(x, y + 60, 300, 20);
            bookMeal[i].setBackground(Color.BLACK);
            bookMeal[i].setForeground(Color.WHITE);
            p2.add(bookMeal[i]);
            bookMeal[i].addActionListener(this);
            y = y + 100;
        }
    }
